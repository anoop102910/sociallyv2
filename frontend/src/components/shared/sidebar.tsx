...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className="sidebar  flex-1 bg-white text-gray-700 appear-animation m-2 rounded-md shadow-md w-64 pt-3 px-1  h-[87vh] max-[940px]:hidden">
      <ul className="flex flex-col gap-y-3">
        {sidebarItems.map(({ href, icon, label }) => (
          <Link key={href} href={href} className={style.sideBtn}>
            {icon}
            <span>{label}</span>
          </Link>
        ))}
